...
export const ProductStats: React.FC<ProductStatsProps> = ({ products }) => {
  ...
  const stats = [
    ...
    { label: 'Connected Nodes', value: supplierCount, icon: RefreshCw, color: 'text-blue-500', bg: 'bg-blue-500/10' },
  ];

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((stat, i) => (
        <div key={i} className="bg-slate-900/40 backdrop-blur-xl border border-white/[0.04] p-5 rounded-2xl">
          ...
          <div className="text-xs text-slate-500 font-semibold tracking-tight">{stat.label}</div>
        </div>
      ))}
    </div>
  );
