  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1">
        <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
      ...
      <div className="space-y-2">
        {announcements.map((msg) => (
          <button 
            ...
            className={cn(
              "w-full text-left bg-slate-900/40 hover:bg-slate-950/60 border border-white/[0.04] p-3 rounded-2xl transition-all flex items-center gap-3 group px-4",
        ...
        {selected && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-slate-950 border border-white/10 rounded-[32px] p-8 shadow-2xl overflow-hidden"
            >
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex justify-between items-start">
                  <div className={cn(
                    "px-3 py-1 rounded-full text-xs font-semibold tracking-tight border",
                ...
                <div className="space-y-4">
                  <h2 className="text-2xl font-semibold text-white uppercase tracking-tight leading-tight">
                    {selected.title}
                  </h2>
                  <div className="p-6 bg-slate-950 rounded-2xl border border-white/[0.04]">
                ...
                <div className="pt-4 flex items-center justify-between border-t border-white/[0.04]">
                  <div className="text-xs font-mono text-slate-400 uppercase tracking-[0.2em]">
                    Sent by SYSTEM // {new Date(selected.createdAt?.seconds * 1000).toLocaleString()}
                  </div>
                  <button 
                    onClick={() => setSelected(null)}
                    className="bg-white text-slate-900 px-6 py-2.5 rounded-xl text-xs font-semibold tracking-tight hover:bg-slate-200 transition-all font-sans"
                  >
                    Acknowledge
                  </button>
                </div>
